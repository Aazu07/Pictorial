    ...
    inner class ImageViewHolder(private val itemBinding: ItemImageRecyclerviewBinding) :
        ...
        fun onBind() {
            val ratio = String.format(
                "%d:%d", list[adapterPosition].width,
                list[adapterPosition].height
            )
            set.clone(itemBinding.parentLayout)
            set.setDimensionRatio(itemBinding.image.id, ratio)
            set.applyTo(itemBinding.parentLayout)
//            val size: Size
